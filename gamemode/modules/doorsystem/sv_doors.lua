...
local function SetDoorOwnable(ply)
    ...
    if not IsValid(ent) or (not ent:isDoor() and not (ent:IsVehicle() or ent.isWacAircraft or ent.LVS)) or ply:GetPos():DistToSqr(ent:GetPos()) > 40000 then
...
local function OwnDoor(ply)
    ...
    if ent:isKeysOwnedBy(ply) then
        local bAllowed, strReason = hook.Call("playerSell" .. ((ent:IsVehicle() or ent.isWacAircraft or ent.LVS) and "Vehicle" or "Door"), GAMEMODE, ply, ent)
        ...
        ent:keysUnOwn(ply)
        ent:setKeysTitle(nil)
        local GiveMoneyBack = math.floor((hook.Call("get" .. ((ent:IsVehicle() or ent.isWacAircraft or ent.LVS) and "Vehicle" or "Door") .. "Cost", GAMEMODE, ply, ent) * 0.666) + 0.5)
        ...
        local iCost = hook.Call("get" .. ((ent:IsVehicle() or ent.isWacAircraft or ent.LVS) and "Vehicle" or "Door") .. "Cost", GAMEMODE, ply, ent)
        if not ply:canAfford(iCost) then
            DarkRP.notify(ply, 1, 4, (ent:IsVehicle() or ent.isWacAircraft or ent.LVS) and DarkRP.getPhrase("vehicle_cannot_afford") or DarkRP.getPhrase("door_cannot_afford"))
            return ""
        end

        local bAllowed, strReason, bSuppress = hook.Call("playerBuy" .. ((ent:IsVehicle() or ent.isWacAircraft or ent.LVS) and "Vehicle" or "Door"), GAMEMODE, ply, ent)
        if bAllowed == false then
            if strReason and strReason ~= "" then
                DarkRP.notify(ply, 1, 4, strReason)
            end
        ...
        local bVehicle = (ent:IsVehicle() or ent.isWacAircraft or ent.LVS)
...
local function UnOwnAll(ply, cmd, args)
    ...
    for entIndex, ent in pairs(unownables) do
        local bAllowed, _strReason = hook.Call("playerSell" .. ((ent:IsVehicle() or ent.isWacAircraft or ent.LVS) and "Vehicle" or "Door"), GAMEMODE, ply, ent)
