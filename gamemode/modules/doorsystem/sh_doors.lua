...
function meta:isKeysOwnable()
    ...
    if (ownableDoors[class] or
            (GAMEMODE.Config.allowvehicleowning and (self:IsVehicle() or self.isWacAircraft or self.LVS) and (not IsValid(self:GetParent()) or not self:GetParent():IsVehicle()))) then
        return true
    end
